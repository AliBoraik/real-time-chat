...
public class MessageHub : Hub
{
    ...
    {
        Console.WriteLine(
            $"!!!!!!!User {message.Username} send message {message.MessageData} to group {groupName}!!!!!!!");
        _producer.SendMessage(message, "ChatApp.Message");
    ...
    public async Task JoinUser(string chatName, string userId)
    {
        ...
        if (chat == null) await _chat.AddChat(Guid.Parse(userId));
